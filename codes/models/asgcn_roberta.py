...
class ASGCN_Bert(nn.Module):
    def __init__(self, bert, opt):
        ...
        self.bert = RobertaModel.from_pretrained(opt.model_name_or_path, return_dict = False)
    ...
    def forward(self, inputs):
        input_ids, token_type_ids, attention_mask, adj, text_indices, aspect_indices, left_indices, = inputs
        # print(input_ids, text_indices)
        # print('len', torch.sum(input_ids != 0, dim=-1), 'xxxxxxx',torch.sum(text_indices != 0, dim=-1))
        # assert False
        # text_indices, aspect_indices, left_indices, adj = inputs
        text_len = torch.sum(input_ids != 0, dim=-1)
        ...
        text_out, _ = self.bert(input_ids, token_type_ids, attention_mask)
